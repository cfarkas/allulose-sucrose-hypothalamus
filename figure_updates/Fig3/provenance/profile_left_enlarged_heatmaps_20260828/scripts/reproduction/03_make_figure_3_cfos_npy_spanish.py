...
ENDPOINTS = (
    (
        "D", "cfos_over_dapi", "Activación total de c-FOS",
        "Núcleos c-FOS positivos (% DAPI)",
        ("cfos_over_dapi", "cfos_dapi_ratio", "cfos_positive_over_dapi"),
    ),
    (
        "E", "double_over_npy", "Activación de c-FOS en núcleos NPY",
        "Núcleos NPY c-FOS positivos (% NPY)",
...
def draw_quantitative_panel(
    ...
    lines = [
        f"MWU exacta {first[0]}–{second[0]} p = {fmt_p(pairs[(first, second)])}"
    ...
    ax.set_title("Campo completo", fontsize=11.5, fontweight="bold", pad=4)
    ax.set_xticks(positions)
    ax.set_xticklabels([DISPLAY_CONDITION_ES[c] for c in CONDITIONS], rotation=24, ha="right", fontsize=10, fontweight="bold")
    ...
    *,
    ...
    for row_index, item in enumerate(rendered):
        ...
        fig.text(
            ...
            f"{DISPLAY_CONDITION_ES[spec['condition']]}\n{spec['sample'].replace('_', ' ')}",
...
def write_text_and_provenance(
    ...
    expected_view_titles = [
        "Núcleos NPY positivos", "Núcleos c-FOS positivos", "Estado combinado",
    ]
    if (
    ...
    panel_descriptions = {
        ...
        "D": "Animal-level Núcleos c-FOS positivos (% DAPI), n=3/3/3.",
        "E": "Animal-level Núcleos NPY c-FOS positivos (% NPY), n=3/3/3.",
    ...
    provenance = {
        ...
        "display_scale": "100 * animal ratio in D/E; source ratios used for ANOVA and MWU exacta",
...
def main() -> int:
    ...
    for column_index, (index, item) in zip((1, 3), enumerate(ENDPOINTS)):
        ...
        endpoint_grid = statistics_row[0, column_index].subgridspec(
            3, 1, height_ratios=(0.40, 2.35, 0.35), hspace=0.02,
        )
        ...
        header_axis.text(
            0.60, 0.95, title, transform=header_axis.transAxes,
