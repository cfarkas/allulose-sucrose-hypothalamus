...
the final paper labels are A–H. No analysis or source measurement is changed.
"""
...
def clean(source):
    ...
    if removed:page.apply_redactions(images=0,graphics=0,text=0)
...
def main():
    ...
    for lang,suffix,folder,spatial in [('en','','final_run','spatial_english'),('es','_spanish','spanish_analysis_work','spatial_spanish')]:
        sources={
          ...
          'G':analysis/spatial/'Figure3_Spatial_cFOS_occurrence.pdf',
          'H':analysis/spatial/'Figure3_Spatial_cFOS_NPY_occurrence.pdf'}
        ...
        for row,fraction in [('A',1),('BCD',1),('EF',.60),('GH',1)]:
            total=(width-2*mar)*fraction;x=(width-total)/2;cell=(total-gap*(len(row)-1))/len(row);heights=[]
            for letter in row:
                page=cleaned[letter][0];h=cell*page.rect.height/page.rect.width
                layout[letter]=fitz.Rect(x,y+labelgap,x+cell,y+labelgap+h);heights.append(h);x+=cell+gap
        ...
        record={'language':lang,'panels':source_records,'panel_boxes_pt':{k:list(v) for k,v in layout.items()},'size_pt':[width,page.rect.height],'dpi':args.dpi,'panel_sequence':list('ABCDEFGH')}
        if lang == 'en':
