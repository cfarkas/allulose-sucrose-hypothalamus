...
def prepare(args: argparse.Namespace) -> int:
    ...
    for _, mrow in manifest.iterrows():
        ...
        sheet_rows.append({
            ...
            "dapi_mip": str(mip_paths["DAPI"]),
            "cfos_mip": str(mip_paths["cFOS"]),
            "acth_clip_mip": str(mip_paths["ACTH_CLIP"]),
            "dapi_seg": str(seg_paths["DAPI"]),
            "cfos_seg": str(seg_paths["cFOS"]),
            "acth_clip_seg": str(seg_paths["ACTH_CLIP"]),
    ...
    for rec in sheet.to_dict("records"):
        vec=dapi_cv_vector(Path(str(rec["dapi_mip"]))); best_id=""; best_corr=-1.0
...
def segment(args: argparse.Namespace) -> int:
    ...
    for _, row in sheet.iterrows():
        ...
        for marker, mip_col, seg_col in [
            ...
            mip = Path(str(row[mip_col])); seg_path = Path(str(row[seg_col]))
...
<title>Figure 5 ARC/ME/VMN review</title><style>
...
<div id="top"><button onclick="loadItem(index-1)">◀</button><button onclick="loadItem(index+1)">▶</button><span id="progress">Loading…</span><select id="mode" onchange="reloadImage()"><option value="seg">DAPI segmentation</option><option value="raw">Raw DAPI</option></select><button onclick="resetView()">Reset view</button><div id="meta"></div><button id="buildBtnTop" class="top-build" onclick="buildFigure()" disabled>Make Figure 5 · 600 dpi</button><button id="stopBtn" onclick="stopServer()" disabled>STOP SERVER</button></div>
...
<div class="section"><button id="buildBtnSide" class="action build-side" onclick="buildFigure()" disabled>Make Figure 5 · 600 dpi</button><div id="buildStatus" class="small">Loading review state…</div></div><p class="small">At least one ARC, ME, or VMN polygon is required for an included field. Figure 5 statistics remain ARC/ME. Shift, middle, or right drag pans; wheel zooms.</p></div></div>
<script>
'use strict';
...
async function stopServer(){let r=await fetch('/api/stop',{method:'POST'});if(r.ok)document.body.innerHTML='<h1 style="padding:40px">Stopped. Figure 5 review is complete.</h1>';else alert(await r.text())}
...
def _display_image(row: Mapping[str, Any], mode: str, max_dim: int) -> Tuple[bytes, Tuple[int, int], Tuple[int, int]]:
    if Image is None:
        raise RuntimeError("Pillow is required for --annotate")
    dapi = maximum_projection(read_stack(Path(str(row["dapi_mip"]))))
    shape = tuple(map(int, dapi.shape))
    if mode == "raw":
        u8 = normalize_u8(dapi)
        rgb = np.repeat(u8[..., None], 3, axis=-1)
    else:
        seg = load_segmentation(Path(str(row["dapi_seg"])))
        if seg.shape != shape:
...
def annotate(args: argparse.Namespace) -> int:
    sheet = read_sheet(args)
    for _, row in sheet.iterrows():
        if not Path(str(row["dapi_seg"])).exists():
    ...
    ensure_dir(paths(args)["annotations"])
    ensure_dir(paths(args)["region_masks"])
    ...
    def rendered(i: int, mode: str) -> Tuple[bytes, Tuple[int, int], Tuple[int, int]]:
        if (i, mode) not in cache:
            cache[(i, mode)] = _display_image(records[i], mode, args.web_max_dim)
        return cache[(i, mode)]
    ...
    def build_route():
        ...
        if result_sentinel.exists():
            return jsonify({
                "error": (
                    f"Refusing to replace existing quantified results: {result_sentinel}. "
                    "Use a fresh FIG5_WORKDIR for a new analysis."
                )
            }), 409
...
def status(args: argparse.Namespace) -> int:
    ...
    if p["sheet"].exists():
        ...
        payload["segmentations"] = {m: int(sum(Path(str(r[c])).exists() for _, r in sheet.iterrows())) for m, c in [("DAPI", "dapi_seg"), ("cFOS", "cfos_seg"), ("ACTH_CLIP", "acth_clip_seg")]}
        states = annotation_states(args, sheet)
...
def audit_segmentations(args: argparse.Namespace) -> int:
    ...
    for _, srow in sheet.iterrows():
        ...
        dapi = load_segmentation(Path(str(srow["dapi_seg"])))
        acth_clip = load_segmentation(Path(str(srow["acth_clip_seg"])))
...
def quantify(args: argparse.Namespace) -> int:
    ...
    for _, srow in sheet.iterrows():
        ...
        annotation_rows.append({"analysis_unit_id":acq,"acquisition_id":acq,"animal_id":srow.get("animal_id",""),"condition":srow.get("condition",""),"status":ann_status,"annotation_json":str(annotation_json_path(args,acq)),"region_mask":str(region_mask_path(args,acq))})
        if ann_status != "include":
            continue
        required = [Path(str(srow[c])) for c in ["dapi_seg","cfos_seg","acth_clip_seg"]]
