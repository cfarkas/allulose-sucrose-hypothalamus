...
including `~/miniforge3`. See [computer setup](SETUP.md) if it is missing. It
creates two project environments at `.paper-envs/main` and `.paper-envs/s3`;
package caches also stay under `.paper-envs/`. The main environment uses the
archived YAML followed by the documented OpenCV correction. The S3 environment
uses the archived explicit Linux lock and pip lock. The numerical thread profile
is applied automatically. Original paper specifications and scripts are checked
against the manifest before use.

For existing custom environments, set `PAPER_PYTHON` and `PAPER_S3_PYTHON` to their
absolute Python executable paths. The program uses those interpreters without
installing packages into them, then checks the same versions and readiness gates.

`logs/01-computer.log`, `logs/02-download.log`, `logs/03-software-main.log`,
`logs/03-software-s3.log`, `logs/04-readiness.log`, and `logs/05-figures.log` contain
the detailed commands and output. Command logs append each attempt. Download
logs include the original verbose URLs, byte counts, hashes, and extraction
steps. Completed downloads and completed software installations are reused on
retry. An interrupted software installation is retried before scientific checks.
If a completed environment was manually changed, restore its documented package
versions before retrying; a preparation receipt does not bypass readiness checks.
